...
import { getPhonemeViseme, MouthSVG } from './MouthShapes';
...
export default function CaraACara({ setView, handleMouseMove }) {
      ...
      if (data.status === 'success') {
        ...
        if (token) {
          ...
          fetch('/api/history', {
            ...
            body: JSON.stringify({ module: 'caraacara', data: JSON.stringify(dataToSave) })
  ...
  // Clicking a word: show SVG phoneme diagrams and save to history
  const getDetails = (wordData) => {
    setSelectedWord(wordData);
    setDetails(null);
    setGeneralError(null);

    // Save phoneme data to history even without AI recommendation
    if (!wordData.correct) {
      const token = localStorage.getItem('token');
      if (token && evaluation) {
        const dataToSave = {
  ...
  const getAIRecommendation = async () => {
    ...
    try {
      ...
      if (result.ok) {
        ...
        // Background update to history to save AI explanation
        const token = localStorage.getItem('token');
        if (token && evaluation) {
          const dataToSave = {
            text: selectedSentence,
